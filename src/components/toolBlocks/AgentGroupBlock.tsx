...
function getStatusIcon(status: SubAgentInfo['status']) {
  switch (status) {
    case 'running':
      return <Loader2 size={12} className="agent-status-running" />;
    case 'completed':
      return <CheckCircle2 size={12} className="agent-status-completed" />;
    case 'error':
      return <XCircle size={12} className="agent-status-error" />;
    default:
      return <span className="agent-status-idle">.</span>;
  }
}
  ...
  return (
    <div className="agent-group-block">
      <div className="agent-group-header" onClick={() => setExpanded(!expanded)}>
        ...
        <span className="agent-group-title">{title || 'Subtasks'}</span>
        <div className="agent-group-stats">
          {runningCount > 0 && <span className="agent-stat running">{runningCount} running</span>}
          {completedCount > 0 && <span className="agent-stat completed">{completedCount} completed</span>}
          {errorCount > 0 && <span className="agent-stat error">{errorCount} errors</span>}
        </div>
